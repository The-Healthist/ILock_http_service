#!/bin/bash
# iLock Manual Deployment Script for macOS - 安全部署版本 (Refactored Project)
...
# 确保服务器上docker-compose.yml不包含platform参数
...
# 创建修改后的docker-compose.yml文件，移除platform参数
...
scp_cmd docker-compose.modified.yml .env backup_script.sh setup_docker_mirror.sh
...
ssh_cmd "cd /root/ilock && chmod +x backup_script.sh && mkdir -p /root/ilock/backups && mv docker-compose.modified.yml docker-compose.yml"
...
# 如果镜像拉取失败，尝试手动拉取每个镜像
ssh_cmd "cd /root/ilock && if ! docker ps | grep -q ilock_http_service; then echo '尝试手动拉取镜像...' && docker pull --platform linux/amd64 stonesea/ilock-http-service:$VERSION && docker pull --platform linux/amd64 mysql:8.0 && docker pull --platform linux/amd64 redis:7.0-alpine && docker pull --platform linux/amd64 eclipse-mosquitto:2.0 && docker-compose up -d $RECREATE_FLAG; fi"
...
PREVIOUS_VERSION="1.3.0"  # 设置为上一个稳定版本
...
# 删除本地备份脚本和临时文件
rm -f setup_docker_mirror.sh docker-compose.modified.yml
